<template>
    <div v-if="dataReady" class="w-100">
      ...
      <div class="chart-container">
        ...
        <div class="year-text" v-if="YearText">
  ...
  onMounted(async () => {
   ...
   const fetchChartData = async (year = 0) => {
     try {
       ...
       if (response(res) === "success") {
         ...
         // تحديث قائمة السنوات من الـ API
         const apiYears = res.data.data.marketData.map(item => ({
           name: item.year.toString(),
           id: parseInt(item.year)
         }));
         
         // تحديث قائمة السنوات مع إبقاء خيار "عرض الكل"
         years.value = [
           { name: 'عرض الكل', id: 0 },
           ...apiYears
         ];
         console.log(apiYears, "4554454554", years.value)
   ...
   // Track displayed years from API
   const displayedYears = computed(() => {
     if (!chartData.value.marketData || chartData.value.marketData.length === 0) {
       return [];
     }
     
     if (!selectedYear.value || selectedYear.value.id === 0) {
       return chartData.value.marketData.map(item => item.year.toString());
     }
     
     const yearId = selectedYear.value.id;
     return chartData.value.marketData
       .filter(item => parseInt(item.year) >= yearId)
   ...
   onMounted(async () => {
     ...
     // تحديد السنة الافتراضية بعد جلب البيانات
